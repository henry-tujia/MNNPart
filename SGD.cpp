...
std::map<Express::VARP, Express::VARP> SGD::onGetNextParameter(Express::VARP loss) {
    ...
    for (auto& iter : grad) {
        ...
        printf("updateValue is %f\n",updateValue->readMap<float>()[0]);
        // apply update
        auto newParameter = iter.first - updateValue;
        printf("newPara is %f\n",newParameter->readMap<float>()[0]);
        iter.second       = newParameter;
    }

    for (auto iter : grad){
        printf("grad first done is %f\n",iter.first->readMap<float>()[0]);
        printf("grad second done is %f\n",iter.second->readMap<float>()[0]);
